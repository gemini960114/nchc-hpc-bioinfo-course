...
## 7. 計費練習：算出一個 job 花了多少錢

送出 SLURM job 不是免費的，每個 partition 都有對應費率（見
[TWCC 手冊第 4 節](../00_environment_setup/TWCC_Taiwania3_GP1生醫節點_使用手冊.md#4-計算節點與費率)）。
知道怎麼算，才知道自己的分析大概要花多少計畫額度，也才看得懂 `wallet` 裡 SU 是怎麼被扣的。

### 7.1 計算公式

```
花費 = 核心小時（Elapsed 時數 × NCPUS） × 費率（元/核心小時）
```

`Elapsed` 和 `NCPUS` 直接從 `sacct` 查：

```bash
sacct -j <jobid> -o JobID,Partition,NCPUS,Elapsed,State
```

### 7.2 實測範例：用我們真的跑過的兩個 job 算一次

**QIIME2 pipeline（job 2030543，`ngs53G`）：**

```
Elapsed = 00:14:44 ≈ 0.2456 小時
NCPUS   = 8
核心小時 = 0.2456 × 8 ≈ 1.965 核心小時
```

`ngs53G` 費率（見手冊第 4 節）：國科會計畫 0.08 元/核心小時、一般學界 0.24 元/核心小時

```
國科會計畫費率：1.965 × 0.08 ≈ 0.16 元
一般學界費率  ：1.965 × 0.24 ≈ 0.47 元
```

**HUMAnN3 pipeline（job 2030736，`ngs53G`）：**

```
Elapsed = 00:01:10 ≈ 0.0194 小時
NCPUS   = 8
核心小時 = 0.0194 × 8 ≈ 0.156 核心小時

國科會計畫費率：0.156 × 0.08 ≈ 0.012 元
一般學界費率  ：0.156 × 0.24 ≈ 0.037 元
```

> 這兩個都是輕量 demo，花費幾乎是零。**重點不是這兩筆錢多少，而是公式本身**——
> 換算成正式研究等級的資料量（例如 DADA2 denoise 跑幾小時、HUMAnN3 用完整資料庫跑幾十小時），
> 核心小時會線性放大，費用差異就會變得很有感，尤其是換到 GPU 或大記憶體節點（費率高出好幾倍）時。
