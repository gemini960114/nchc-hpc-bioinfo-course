...
| MultiQC 執行出現無害的 `_ARRAY_API not found` 警告 | [01_FastQC_MultiQC_教學.md](01_fastqc_multiqc/01_FastQC_MultiQC_教學.md) 第 6 節 |
...
| **（新增）新增兩模組後總時數超過原訂 6 小時** | 加入 FastQC（30分）與 Singularity/HUMAnN3（45分）後，總時數來到約 7.75 小時 | 已拍板採用**一天半**，全部模組排入課堂；場地受限時的一天壓縮備援方案保留於 §3（見 §3、§4） | ✅ 已拍板 |
...
- **總時數（已拍板）：一天半**，約 7.75 小時，全部 9 個 Module 都排進課堂時段：
  - **第一天**：Module 0~6（環境準備 → 認識平台 → 建立 QIIME2 環境 → notebook 轉寫 → FastQC/MultiQC → 完整跑一次 Moving Pictures → SLURM 排程實戰）
  - **第二天上午**：Module 7（Singularity/HUMAnN3）+ Module 8（總結與 Q&A）
  - **決策理由**：Module 4、7 的教材品質已經跟其他模組一樣完整（含真實除錯案例、已實測驗證），
    壓縮成選修自學會讓這兩塊的學習效果打折扣，且落差不大（多一個上午即可），優先維持課堂教學品質。
  - **備援方案**：若場地/人力只能給一天，改把 Module 4、7 列為**課後指定自學**（教材本身可獨立自學，
    不需要現場帶），課堂只上 Module 0~3、5、6、8，壓回約 6 小時——這個備援方案保留，開課前一週視實際場地確認。
